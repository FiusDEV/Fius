...
import type { SearchDisplayData, Tool, ToolExecutionContext } from '@fius/core/tools';
    ...
    return defineTool({
        ...
        async execute(input, context: ToolExecutionContext) {
            ...
            if (isOutsideWorkspace) {
                ...
                const fullPath = nodePath.join(resolvedSearchPath, pattern);
                matches = await glob(fullPath, {
                    nodir: true,
                    absolute: true,
                ...
                const workspacePattern = joinWorkspacePattern(
                    handle.context.path,
                    searchPath,
                    pattern
                );
                matches = await handle.files.glob(workspacePattern);
            }

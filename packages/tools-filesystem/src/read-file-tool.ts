    ...
    return defineTool({
        ...
        async execute(input, context: ToolExecutionContext) {
            ...
            if (workspacePath.startsWith('..')) {
                // Path is outside workspace - read directly via fs (approval already granted)
                const buffer = await fs.readFile(input.file_path, 'utf-8');
                content = buffer;
            } else {
                if (typeof handle.files.readText !== 'function') {
                    throw new Error(`Workspace file read unavailable: ${input.file_path}`);
                }
                content = await handle.files.readText(workspacePath);
            }

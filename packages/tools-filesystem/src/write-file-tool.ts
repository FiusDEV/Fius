...
export function createWriteFileTool(
    ...
    return defineTool({
        ...
        presentation: {
            ...
            preview: async (input, context: ToolExecutionContext) => {
                ...
                if (workspaceManager !== undefined) {
                    ...
                    const isOutsideWorkspace = workspacePath.startsWith('..');
                    try {
                        const originalContent = isOutsideWorkspace
                            ? await fs.readFile(file_path, 'utf-8')
                            : await handle.files.readText(workspacePath);
        ...
        async execute(input, context: ToolExecutionContext) {
            ...
            try {
                originalContent = isOutsideWorkspace
                    ? await fs.readFile(file_path, 'utf-8')
                    : await handle.files.readText(workspacePath);
                fileExistsNow = true;
            } catch (error) {
            ...
            if (isOutsideWorkspace) {
                const dir = file_path.substring(0, file_path.lastIndexOf('/')) || file_path.substring(0, file_path.lastIndexOf('\\'));

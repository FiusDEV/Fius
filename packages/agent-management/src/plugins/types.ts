 ...
 * - user: Installed to ~/.fius/plugins/<name>/
 * - project: Installed to <cwd>/.fius/plugins/<name>/
 * - local: Registered in-place (no copy)
 */
export type PluginInstallScope = 'user' | 'project' | 'local';

/**
 * Entry in installed_plugins.json for Fius's plugin tracking
 */
...
export interface ListedPlugin {
    ...
    /** Absolute path to plugin directory */
    path: string;
    /** Source of plugin discovery (always 'fius' now) */
    source: 'fius';
    /** Installation scope if installed via Fius */

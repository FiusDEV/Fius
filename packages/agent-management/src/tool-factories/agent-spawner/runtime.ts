...
import { getFiusPath, resolveBundledScript } from '../../utils/path.js';
...
export class AgentSpawnerRuntime implements TaskForker {
    ...
    private async getProjectRootCandidates(): Promise<string[]> {
        ...
        for (const seedPath of seedPaths) {
            ...
            const detectedProjectRoot = findFiusProjectRoot(seedPath);
    ...
    private setupProgressTracking(
        subAgentHandle: { agentId: string; agent: FiusAgent },
     ...
     */
    private async applyParentWorkspace(agent: FiusAgent): Promise<void> {
        let parentWorkspace: Awaited<ReturnType<FiusAgent['getWorkspace']>>;
        try {
    ...
    private async trySpawnWithFallback(
        ...
        try {
            ...
            let handle: { agentId: string; agent: FiusAgent };

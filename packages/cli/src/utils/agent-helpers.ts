 ...
 * Points to ~/.fius/agents/registry.json
 */
 ...
 *
 * This manager operates on the global agents directory (~/.fius/agents)
 * and uses the user registry file (~/.fius/agents/registry.json).
 *
...
export function getCLIAgentManager(): AgentManager {
    if (cliAgentManager === null) {
        const registryPath = path.join(getFiusGlobalPath('agents'), 'registry.json');
...
/**
 * Install bundled agent from registry to ~/.fius/agents
 *
 * @param agentId ID of the agent to install from bundled registry
 * @param options Installation options (agentsDir defaults to ~/.fius/agents)
...
/**
 * Install custom agent from local path to ~/.fius/agents
 *
...
/**
 * List installed agents from ~/.fius/agents
 *
 * @param options Installation options (agentsDir defaults to ~/.fius/agents)

...
export async function runWebMode(context: MainModeContext): Promise<void> {
    ...
    const serverUrl = process.env.FIUS_URL ?? `http://localhost:${port}`;

    const webRoot = resolveWebRoot();
    if (!webRoot) {
        console.warn(chalk.yellow('вљ пёЏ  WebUI not found in this build.'));
    ...
    console.log(chalk.green(`вњ… Server running at ${serverUrl}`));
    ...
    if (webRoot) {
        try {
            ...
            console.log(chalk.green(`рџЊђ Opened WebUI in browser: ${serverUrl}`));
        } catch (_error) {
            console.log(chalk.yellow(`рџ’Ў WebUI is available at: ${serverUrl}`));
        }
    }
}
...
export async function dispatchMainMode(context: MainModeContext): Promise<void> {
    ...
    try {
        switch (mode) {
            ...
            default:
                if (mode === 'discord' || mode === 'telegram') {
                    console.error(`вќЊ Error: '${mode}' mode has been moved to examples`);
                    console.error('');
                    console.error(
                        `The ${mode} bot is now a standalone example that you can customize.`
                    );
                    console.error('');
                    console.error(`рџ“– See: examples/${mode}-bot/README.md`);
                    console.error('');
                    ...
                    console.error(`вќЊ Unknown mode '${mode}'. Use web, cli, server, or mcp.`);
                }
        ...
        console.error(`вќЊ Failed to start '${mode}' mode: ${message}`);

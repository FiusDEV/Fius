...
export type DeviceCodePollResponse =
    ...
    | { status: 'approved'; apiKey: DeviceCodeApiKeyResponse; userEmail?: string };
...
export class FiusApiClient {
    ...
    private async createPlatformApiKey(
        authToken: string,
        name: string
    ): Promise<{ fiusApiKey: string; keyId: string }> {
        const response = await fetch(this.getPlatformUrl('/api/keys'), {
            method: 'POST',
            headers: {
                Authorization: `Bearer ${authToken}`,
        ...
        if (!result.success || !result.data) {
            throw new Error(result.error || 'Failed to create Fius API key');
        }

        return {
            fiusApiKey: result.data.fullKey,
    ...
    /**
     * Validate if a Fius API key is valid
     */
    async validateFiusApiKey(apiKey: string): Promise<boolean> {
        try {
            logger.debug('Validating FIUS_API_KEY');
            ...
            logger.error(`Error validating Fius API key: ${error}`);
            return false;
        }
    }

    /**
     * Provision Fius API key and always return a usable secret key value.
     * @param regenerate - If true, delete existing key(s) and create a new one
     */
    async provisionFiusApiKey(
        authToken: string,
        name: string = 'Fius CLI Key',
        regenerate: boolean = false
    ): Promise<{ fiusApiKey: string; keyId: string }> {
        try {
            logger.debug(
                `Provisioning FIUS_API_KEY with name: ${name}, regenerate: ${regenerate}`
            );
            ...
            logger.debug(`Successfully provisioned FIUS_API_KEY: ${createdKey.keyId}`);
            return {
                fiusApiKey: createdKey.fiusApiKey,
                keyId: createdKey.keyId,
            };
        } catch (error) {
            logger.error(`Error provisioning Fius API key: ${error}`);
            throw error;
        }
    }
    ...
    /**
     * Get user plan and allowed models from the Fius platform (CLI endpoint).
     * Returns plan info and the list of models the user can access.
     */
    async getCliUserInfo(apiKey: string): Promise<CliUserInfo> {
        try {
            logger.debug('Fetching CLI user info');

            const response = await fetch(this.getPlatformUrl('/api/cli/user'), {
                method: 'GET',
                headers: {
                    Authorization: `Bearer ${apiKey}`,
                },
            ...
            const plan = parseString(Reflect.get(payload, 'plan'));
            const allowedModels = parseStringArray(Reflect.get(payload, 'allowed_models'));
            ...
            logger.error(`Error fetching CLI user info: ${error}`);
            throw error;
        }
    }

    /**
     * Get the current Fius billing balance for the authenticated user.
     */
    ...
    async startDeviceCodeLogin(
        client: string = 'fius-cli',
        options: RequestOptions = {}
    ): Promise<DeviceCodeStartResponse> {
        const response = await fetch(this.getPlatformUrl('/api/cli/auth/start'), {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
        ...
        if (!response.ok) {
            if (response.status === 404 || response.status === 403) {
                throw new Error('Device code login is not enabled for this Fius server');
            }
        ...
        return {
            deviceCode: payload.session || '',
            userCode: '',
            verificationUrl: payload.url || '',
            verificationUrlComplete: payload.url || null,
    ...
    async pollDeviceCodeLogin(
        ...
        try {
            response = await fetch(this.getPlatformUrl('/api/cli/auth/check'), {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({ session: deviceCode }),
                signal: this.createRequestSignal(options.signal),
            });
        } catch (error) {
        ...
        if (!response.ok) {
            ...
            const errorCode = payload?.error || payload?.error_description;
            if (errorCode) {
        ...
        if (payload?.status === 'approved') {
            ...
            if (token) {
                const keyId = token.substring(0, 8);
                return {
                    status: 'approved',
                    apiKey: {
                        id: keyId,
                    ...
                    userEmail: payload.user_email || undefined,
                };
            }
        }
...
export function getFiusApiClient(): FiusApiClient {
    return new FiusApiClient();
}
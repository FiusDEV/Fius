...
function formatCloudAgentListItem(
    ...
    const prefix = linkedToWorkspace ? chalk.green('в†’') : ' ';
...
async function selectCloudAgentForChat(workspaceRoot: string): Promise<string> {
    ...
    try {
        ...
        spinner.stop(chalk.red('вњ— Failed to load cloud agents'));
        throw error;
    }

    spinner.stop(chalk.green('вњ“ Cloud agents loaded'));

    if (cloudAgents.length === 0) {
        throw new Error(
            'No cloud agents available yet. Run `fius deploy` first, or pass a cloud agent ID explicitly.'
        );
    }
...
export async function handleDeployCommand(options?: InteractiveOptions): Promise<void> {
    ...
    try {
        ...
        spinner.stop(chalk.green('вњ“ Workspace deployed'));
        p.outro(
            [
                ...
                ...(linkSyncError
                    ? [
                          ...
                          'Run `fius deploy` again in this workspace to re-link.',
                      ]
                    : []),
                '',
                'Next steps:',
                `- Open the dashboard to inspect and interact with the deployment`,
                `- Run \`fius deploy status\` from this workspace`,
            ].join('\n')
        );
    } catch (error) {
        spinner.stop(chalk.red('вњ— Deploy failed'));
        throw error;
    } finally {
        if (snapshot) {
...
export async function handleDeployListCommand(): Promise<void> {
    ...
    if (cloudAgents.length === 0) {
        p.outro(
            'No cloud deployments found.\nRun `fius deploy` from this workspace to create one.'
        );
        return;
    }
...
export async function handleDeployOpenCommand(): Promise<void> {
    ...
    if (!deployLink) {
        throw new Error(
            'This workspace is not linked to a cloud deployment yet. Run `fius deploy` first.'
        );
    }
...
export async function handleDeployStopCommand(): Promise<void> {
    ...
    try {
        ...
        spinner.stop(chalk.green('вњ“ Cloud sandbox stopped'));
        p.outro(
            [
        ...
        spinner.stop(chalk.red('вњ— Stop failed'));
        throw error;
    }
}
...
export async function handleDeployDeleteCommand(options?: InteractiveOptions): Promise<void> {
    ...
    try {
        ...
        spinner.stop(chalk.green('вњ“ Cloud deployment deleted'));
        p.outro(
            linkRemoveError
        ...
        spinner.stop(chalk.red('вњ— Delete failed'));
        throw error;
    }
}

const APP_URL_ENV_VAR = 'FIUS_APP_URL';
const SANDBOX_URL_ENV_VAR = 'FIUS_SANDBOX_URL';
const DEFAULT_APP_URL = 'https://fius.dev';
...
export function resolveDashboardBaseUrl(): string {
    ...
    if (
        ...
        isLocalHostUrl(process.env.FIUS_API_URL)
    ) {
        return LOCAL_DASHBOARD_URL;
    }

    const normalizedApiUrl = normalizeBaseUrl(process.env.FIUS_API_URL);
    if (normalizedApiUrl) {
        const apiUrl = new URL(normalizedApiUrl);
        if (apiUrl.hostname === 'api.fius.ai') {

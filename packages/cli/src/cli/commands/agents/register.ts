...
export function registerAgentsCommand({ program }: AgentsCommandRegisterContext): void {
    ...
    agentsCommand
        ...
        .addHelpText(
            ...
            `
Examples:
  $ fius agents install coding-agent               Install agent from registry
  $ fius agents install agent1 agent2              Install multiple registry agents
  $ fius agents install --all                      Install all available registry agents
  $ fius agents install ./my-agent.yml             Install custom agent from YAML file
  $ fius agents install ./my-agent-dir/            Install custom agent from directory (interactive)`
        )
        .action(
            withAnalytics(
                'agents install',
                async (agents: string[] = [], options: Partial<InstallCommandOptions>) => {
                    try {
                        ...
                        console.error(`вќЊ fius agents install command failed: ${err}`);
            ...
            withAnalytics(
                ...
                async (agents: string[], options: Partial<UninstallCommandOptions>) => {
                    try {
                        ...
                        if (err instanceof ExitSignal) throw err;
                        console.error(`вќЊ fius agents uninstall command failed: ${err}`);
        ...
        .action(
            withAnalytics('agents list', async (options: ListAgentsCommandOptionsInput) => {
                try {
                    ...
                    if (err instanceof ExitSignal) throw err;
                    console.error(`вќЊ fius agents list command failed: ${err}`);
        ...
        .action(
            withAnalytics('agents sync', async (options: Partial<SyncAgentsCommandOptions>) => {
                try {
                    ...
                    if (err instanceof ExitSignal) throw err;
                    console.error(`вќЊ fius agents sync command failed: ${err}`);

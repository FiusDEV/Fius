...
export async function handleImageInstallCommand(
    ...
    console.log(
        chalk.green(
            `вњ“ ${result.installMode === 'linked' ? 'Linked' : 'Installed'} ${result.id}@${result.version}`
        )
    );
...
export async function handleImageListCommand(): Promise<void> {
    ...
    if (ids.length === 0) {
        ...
        console.log(chalk.cyan('  fius image install <package-or-path>'));
        return;
    }
...
export async function handleImageUseCommand(options: { image: string }): Promise<void> {
    ...
    console.log(chalk.green(`вњ“ Set active ${parsed.id}@${parsed.version}`));
}
...
export async function handleImageRemoveCommand(options: { image: string }): Promise<void> {
    ...
    console.log(
        chalk.green(
            parsed.version
                ? `вњ“ Removed ${parsed.id}@${parsed.version}`
                : `вњ“ Removed ${parsed.id} (all versions)`
        )
    );
}

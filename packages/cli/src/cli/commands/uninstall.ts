...
export async function handleUninstallCliCommand(
    ...
    const fiusHomePath = getFiusHomePath();
    ...
    if (detection.method === 'npm') {
        ...
        console.log('рџ§№ Uninstalling CLI via npm...');
        try {
            ...
            console.warn(`вљ пёЏ  Package-manager uninstall failed: ${managedUninstallError.message}`);
        }
    } else {
        ...
        try {
            if (
                ...
                const deferredTargets = validated.purge
                    ? [binaryPath, fiusHomePath]
                    : [binaryPath];
                await scheduleDeferredWindowsRemoval(deferredTargets);
                homeRemovalDeferred = validated.purge;
                console.log('рџ—‘пёЏ  Scheduled CLI binary removal after exit.');
                if (validated.purge) {
                    console.log(`рџ—‘пёЏ  Scheduled ${fiusHomePath} removal after exit.`);
                }
            } else {
                const removedBinaryPaths = await removeTargets([binaryPath], validated.dryRun);
                if (removedBinaryPaths.length > 0) {
                    console.log(`рџ—‘пёЏ  Removed CLI binary: ${removedBinaryPaths.join(', ')}`);
                } else {
                    console.log('в„№пёЏ  CLI binary was not found at expected path.');
                }
            }
        } catch (error) {
            binaryRemovalError = error instanceof Error ? error : new Error(String(error));
            console.warn(`вљ пёЏ  Binary removal failed: ${binaryRemovalError.message}`);
        }
    }

    if (validated.purge && !homeRemovalDeferred) {
        const removedHomePaths = await removeTargets([fiusHomePath], validated.dryRun);
        if (removedHomePaths.length > 0) {
            console.log(`рџ—‘пёЏ  Removed ${fiusHomePath}.`);
        } else {
            console.log(`в„№пёЏ  ${fiusHomePath} was not found.`);
        }
    } else if (!validated.purge) {
        console.log(`в„№пёЏ  Keeping ${fiusHomePath}.`);
    }

    if (validated.dryRun) {
        console.log('вњ… Dry run completed. No files were deleted.');
        return;
    }
    ...
    console.log('вњ… Fius CLI uninstall completed.');
}
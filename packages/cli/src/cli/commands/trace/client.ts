import { z } from 'zod';
import { FIUS_PLATFORM_URL } from '../../auth/constants.js';
import { getFiusApiKey } from '../../auth/service.js';
...
export function resolveTracePlatformUrl(platformUrl?: string): string {
    const rawUrl = platformUrl?.trim() || FIUS_PLATFORM_URL;
    if (rawUrl.trim().length === 0) {
        throw new Error('Fius platform URL is empty.');
    }
...
async function resolveTraceApiKey(): Promise<string> {
    ...
    if (!apiKey?.trim()) {
        throw new Error('Authentication required. Run `fius login` before using `fius trace`.');
    }

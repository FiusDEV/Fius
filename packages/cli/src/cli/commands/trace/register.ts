...
export function registerTraceCommand({ program }: TraceCommandRegisterContext): void {
    const trace = program
        .command('trace [runId]')
        .description('Discover and fetch hosted run traces from Fius Cloud')
        .option('--json', 'Print the raw trace response as JSON')
        .option('--platform-url <url>', 'Use a custom Fius platform URL')
        .action(
            withAnalytics('trace', async (runId: string, options: TraceCommonOptions) => {
                try {
                    ...
                    console.error(`вќЊ fius trace command failed: ${err}`);
        ...
        .action(
            withAnalytics('trace:list', async (options) => {
                try {
                    ...
                    if (err instanceof ExitSignal) throw err;
                    console.error(`вќЊ fius trace list command failed: ${err}`);
        ...
        .action(
            withAnalytics('trace:view', async (runId: string, options: TraceCommonOptions) => {
                try {
                    ...
                    if (err instanceof ExitSignal) throw err;
                    console.error(`вќЊ fius trace view command failed: ${err}`);

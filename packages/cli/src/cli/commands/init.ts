...
const DEFAULT_AGENTS_MD = `<!-- fius-workspace -->
...
- Use \`.fius/\` only for Fius-managed state and installed assets

## Defaults
- If no workspace agent is defined, Fius uses your global default agent locally
...
const AGENT_PROMPT_GENERATOR_SYSTEM_PROMPT = [
    'You design production-grade system prompts for Fius agents.',
...
function buildPromptGenerationRequest(
    ...
    return [
        'Generate a Fius system prompt from this brief.',
        '',
...
async function generateAgentSystemPromptFromDescription(
    ...
    if (!effectiveLLM) {
        throw new Error(
            'No active LLM configuration is available for prompt generation. Configure one with `fius setup` first.'
        );
    }
    ...
    const generatorAgent = await createFiusAgentFromConfig({
...
async function editSystemPromptInEditor(
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'fius-agent-prompt-'));
...
async function promptForAvailableAgentName(workspaceRoot: string): Promise<AgentWizardIdentity> {
    ...
    while (true) {
        ...
        p.log.warn(
            `Agent '${identity.agentId}' already exists in ${path.relative(resolvedWorkspaceRoot, registryState.path)}. Choose a different name or run \`fius init agent ${identity.agentId}\` to update the existing agent.`
        );
...
async function promptForAgentSystemPrompt(
    ...
    if (!effectiveLLM) {
        p.log.info(
            'No active LLM configuration found. Opening the prompt editor instead. Run `fius setup` to enable automatic prompt generation.'
        );
    ...
    let roleDescription = await textOrExit(
        {
            message: 'Describe this agentвЂ™s role',
...
description: "Create or update Fius skill bundles with SKILL.md, handlers, scripts, mcps, and references."
---

# Create Skill

Create or update standalone Fius skill bundles. Treat \`skills/<id>/\` as the canonical workspace location unless the user explicitly asks for a global skill.
    ...
    if (!existingEntry) {
        throw new Error(
            `Agent '${agentId}' is not registered in ${path.relative(
                workspace.root,
                registryState.path
            )}. Run \`fius init agent ${agentId}\` first or update the registry manually.`
        );
    }
...
function describeEffectiveDeployAgent(input: {
    ...
    if (input.deployConfig) {
        const configuredAgent = isWorkspaceDeployAgent(input.deployConfig.agent)
            ...
            : 'default cloud agent';
        return `${configuredAgent} via ${input.deployConfigPath ?? '.fius/deploy.json'}`;
    }

    if (input.implicitWorkspaceAgent) {
        return `workspace agent (${input.implicitWorkspaceAgent}) if you run \`fius deploy\``;
    }

    return 'default cloud agent if you run `fius deploy`';
}
...
async function resolvePrimaryAgentSelection(
    ...
    if (primaryCandidates.length === 0) {
        throw new Error('No primary-capable workspace agents found. Run `fius init agent` first.');
    }
...
export async function handleInitCommand(workspaceRoot: string = process.cwd()): Promise<void> {
    p.intro(chalk.inverse('Fius Init'));
...
export async function handleInitAgentCommand(
    ...
    p.intro(chalk.inverse('Fius Init Agent'));
        ...
        chalk.green(
            [
                ...
                ...(subagentLinkResult?.status === 'set' && subagentLinkResult.parentAgentId
                    ...
                    : subagentLinkResult?.status === 'no-primary'
                      ? [
                            'No primary agent found to link this subagent. Use `fius init primary <id>` and rerun if needed.',
                        ]
                      : []),
            ].join('\n')
        )
...
export async function handleInitSkillCommand(
    ...
    p.intro(chalk.inverse('Fius Init Skill'));
...
export async function handleInitPrimaryCommand(
    ...
    p.intro(chalk.inverse('Fius Init Primary'));
...
export async function handleInitStatusCommand(
    ...
    p.intro(chalk.inverse('Fius Init Status'));
...
export function registerInitCommand({ program }: InitCommandRegisterContext): void {
    const initCommand = program
        .command('init')
        .description('Initialize the current folder as a Fius workspace');
...
Examples:
  $ fius init
  $ fius init agent
  $ fius init agent explore-agent --subagent
  $ fius init primary review-agent
  $ fius init skill code-review
  $ fius init status
`
    );
    ...
    initCommand.action(
        withAnalytics('init', async () => {
            try {
                ...
                console.error(`вќЊ fius init command failed: ${err}`);
            ...
            withAnalytics(
                ...
                async (id: string | undefined, options: InitAgentCommandOptions) => {
                    try {
                        ...
                        if (err instanceof ExitSignal) throw err;
                        console.error(`вќЊ fius init agent command failed: ${err}`);
        ...
        .action(
            withAnalytics('init primary', async (id: string | undefined) => {
                try {
                    ...
                    if (err instanceof ExitSignal) throw err;
                    console.error(`вќЊ fius init primary command failed: ${err}`);
        ...
        .action(
            withAnalytics('init skill', async (id: string) => {
                try {
                    ...
                    if (err instanceof ExitSignal) throw err;
                    console.error(`вќЊ fius init skill command failed: ${err}`);
        ...
        .action(
            withAnalytics('init status', async () => {
                try {
                    ...
                    if (err instanceof ExitSignal) throw err;
                    console.error(`вќЊ fius init status command failed: ${err}`);

...
async function validateInstalledImageModule(
    ...
    try {
        ...
        throw new Error(
            `Installed image '${imageId}@${version}' did not export a valid FiusImage.\n` +
        ...
        if (message.startsWith('Image entry file not found:')) {
            throw new Error(
                `Local image '${imageId}@${version}' has not been built.\n` +
                    `${message}\n` +
                    `Run: pnpm run build (or npm run build) in ${packageDir}, then re-run: fius image install ${packageDir}`
            );
        }
        throw error;
    }
...
export async function installImageToStore(
    ...
    try {
        ...
        await fs.writeFile(
            tmpPackageJsonPath,
            JSON.stringify({ name: 'fius-image-store', private: true }, null, 2),
            'utf-8'
        );
        ...
        if (isFileLikeImageSpecifier(specifier) && (await isDirectory(installSpecifier))) {
            packDir = path.join(tmpDir, '.fius-pack');

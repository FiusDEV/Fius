...
const agent = new FiusAgent({
    ...
    logger,
    stores: new InMemoryFiusStores(logger),
});
...
const { stop } = await startFiusServer(agent, {
    port: 3000,
...
async function init() {
    try {
        ...
        addMessage('assistant', "Hello! I'm your Fius assistant. How can I help you today?");
    } catch (error) {
...
function addMessage(role, content, isLoading = false) {
    ...
    avatar.textContent = role === 'user' ? 'рџ‘¤' : 'рџ¤–';
...
export function generateFiusImageFile(context: TemplateContext): string {
...
A **Fius image** is a distributable npm module that exports a typed \`FiusImage\` (a plain object)
describing tool/storage/hook/compaction factories + optional default config.

## What's Included

This package contains:
- вњ… Factories (auto-discovered from convention-based folders)
- вњ… Optional defaults (\`image.defaults\`) that merge into agent config (config wins)
...
# Install into the Fius CLI (local)
npm pack
fius image install ./<generated-file>.tgz
\`\`\`

## Usage

Set \`image: '${imageName}'\` in your agent config (or pass \`--image\` in the CLI), then run Fius.
...
This runs \`fius-bundle build\`, which:
1. Discovers factories from convention-based folders
2. Compiles factory source files to \`dist/\`
3. Generates \`dist/index.js\` exporting a \`FiusImage\` (no side effects)
	...
	const ConfigSchema = z
	    .object({
	        ...
	        thresholdPercent: z
	            ...
	            .max(1.0)
	            .default(0.9)
	            .describe('Trigger threshold (0вЂ“1)'),
	    })
	    .strict();
...
- [Fius Documentation](https://docs.fius.ai)
- [Agent Configuration Guide](https://docs.fius.ai/docs/guides/configuration)
`;
}
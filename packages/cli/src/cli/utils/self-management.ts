...
const FIUS_BINARY = process.platform === 'win32' ? 'fius.exe' : 'fius';
const FIUS_PATH_COMMAND = process.platform === 'win32' ? 'where' : 'which';
const FIUS_PATH_ARGS = process.platform === 'win32' ? ['fius'] : ['-a', 'fius'];
const DEFAULT_NATIVE_INSTALL_URL = 'https://fius.dev/install';
const DEFAULT_WINDOWS_INSTALL_URL = 'https://fius.dev/install.ps1';
...
export function getFiusHomePath(): string {
    return path.join(os.homedir(), '.fius');
}

export async function readInstallMetadata(): Promise<InstallMetadata | null> {
    const metadataPath = path.join(getFiusHomePath(), 'install.json');
...
async function getBinaryPathsFromPath(): Promise<string[]> {
    const result = await executeCommand(FIUS_PATH_COMMAND, FIUS_PATH_ARGS);
    if (result.code !== 0) {
        return [];
    }
...
function inferInstallMethodFromPath(binaryPath: string): InstallMethod {
    ...
    if (normalized.includes('.fius') || normalized.includes(path.join('.local', 'bin'))) {
        return 'native';
    }
...
export function isProjectLocalBinaryPath(binaryPath: string): boolean {
    ...
    return (
        normalized.includes('/node_modules/.bin/') || normalized.includes('/node_modules/fius/')
    );
}
...
function pathBasenameIsFius(targetPath: string): boolean {
    const basename = path.basename(targetPath).toLowerCase();
    return basename === 'fius' || basename === 'fius.exe' || basename === 'fius.cmd';
}
...
export function buildMultipleInstallWarning(
    ...
    return [
        'Multiple fius binaries detected in PATH:',
        formattedPaths,
...
export async function detectInstallMethodWithDeps(
    ...
    async function detectMethodFromActivePath(pathEntry: string): Promise<InstallMethod> {
        ...
        if (method === 'unknown' && pathBasenameIsFius(pathEntry)) {
...
export function createNativeInstallCommand(options: {
    ...
    if (options.version) {
        env.FIUS_VERSION = options.version;
        envOverrides.FIUS_VERSION = options.version;
    }

    if (options.installDir) {
        env.FIUS_INSTALL_DIR = options.installDir;
        envOverrides.FIUS_INSTALL_DIR = options.installDir;
    }

    if (options.force) {
        env.FIUS_INSTALL_FORCE = '1';
        envOverrides.FIUS_INSTALL_FORCE = '1';
    }
...
export function createLegacyNpmUninstallCommand(): ExecutableCommand {
    return {
        command: 'npm',
        args: ['uninstall', '-g', 'fius'],
        displayCommand: 'npm uninstall -g fius',
    };
}
...
export function normalizeRequestedVersion(version: string | null | undefined): string | null {
    ...
    return trimmed.startsWith('fius@') ? trimmed.slice('fius@'.length) : trimmed;
}
...
export function getDefaultNativeBinaryPath(): string {
    if (process.platform === 'win32') {
        return path.join(os.homedir(), '.fius', 'bin', FIUS_BINARY);
    }

    return path.join(os.homedir(), '.local', 'bin', FIUS_BINARY);
}
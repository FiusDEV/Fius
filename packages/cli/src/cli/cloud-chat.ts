...
type KnownProvider = ReturnType<FiusAgent['getCurrentLLMConfig']>['provider'];
...
function contentInputToParts(input: Parameters<CloudChatBackend['stream']>[0]): ContentPart[] {
    return typeof input === 'string' ? [{ type: 'text', text: input }] : [...(input as any)];
}
...
async function resolveCloudAgentId(options: StartCloudChatCliOptions): Promise<string> {
    ...
    throw new Error(
        'No cloud agent ID provided and this workspace is not linked to a deployment. Run `fius deploy` first or pass a cloud agent ID.'
    );
}
...
export function createCloudAgentBackend(
    ...
    const defaultLogLevel: FiusAgent['logger']['getLevel'] = () => {
        return logger.getLevel() as ReturnType<FiusAgent['logger']['getLevel']>;
    };
    ...
    const currentLLMConfig = (sessionId?: string): LLMConfig => {
        ...
        return {
            provider: cachedModel?.provider ?? 'openrouter',
    ...
    const backend: CloudChatBackend = {
        ...
        deleteSession: (async () => {
            throw unsupportedCloudFeature('Session deletion');
        }) as CloudChatBackend['deleteSession'],

        clearSessionLLMOverride: (async () => {}) as CloudChatBackend['clearSessionLLMOverride'],

        setPermissionsMode: (async () => {
            throw unsupportedCloudFeature('Permission mode');
        }) as CloudChatBackend['setPermissionsMode'],

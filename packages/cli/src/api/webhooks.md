...
The Fius webhook system provides HTTP-based event delivery for agent events, offering an alternative to SSE streams for cloud integrations.

## Overview

Webhooks allow you to receive real-time notifications about agent events by registering HTTP endpoints that will receive POST requests when events occur. This is similar to how Stripe webhooks work - when something happens in your Fius agent, we'll send a POST request to your configured webhook URL.
...
```typescript
interface FiusWebhookEvent<T extends AgentEventName = AgentEventName> {
...
{
    "url": "https://your-app.com/webhooks/fius",
...
When you provide a `secret` during webhook registration, Fius will include an HMAC signature in the `X-Fius-Signature-256` header for verification:

```
X-Fius-Signature-256: sha256=a1b2c3d4e5f6...
```
...
app.post('/webhooks/fius', async (c) => {
    const signature = c.req.header('x-fius-signature-256');
...
- `X-Fius-Event-Type: {event_type}`
- `X-Fius-Event-Id: {event_id}`
- `X-Fius-Delivery-Attempt: {attempt_number}`
- `X-Fius-Signature-256: sha256={signature}` (if secret provided)
...
3. **Verify Signatures**: Always verify webhook signatures in production to ensure requests are from Fius.
...
import type { FiusWebhookEvent } from '@fius/server';

const app = new Hono();

app.post('/webhooks/fius', async (c) => {
    // Parse JSON from request body
    const event: FiusWebhookEvent = await c.req.json();
...
Webhooks are only available when Fius is running in server mode (`fius --mode server` command). They are not available in CLI or other modes since webhooks require the HTTP API server to be running.

...
import { ESC, FOCUS_IN, FOCUS_OUT, SGR_EVENT_PREFIX, X11_EVENT_PREFIX } from '../utils/input.js';
...
export function KeypressProvider({ children }: KeypressProviderProps) {
    ...
    useEffect(() => {
        ...
        const baseDataListener = createDataListener(pasteBufferer);

        // Skip mouse escape sequences - MouseProvider handles them independently
        const mousePrefixes = [SGR_EVENT_PREFIX, X11_EVENT_PREFIX];
        const dataListener = (data: string) => {
            if (mousePrefixes.some((prefix) => data.startsWith(prefix))) {
                return; // Mouse data - skip, MouseProvider will handle it
            }
            baseDataListener(data);
        };

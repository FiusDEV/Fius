...
function getXFiusStepLabel(prop: Record<string, unknown>): string | undefined {
    const xFius = prop['x-fius'];
    if (!xFius || typeof xFius !== 'object' || Array.isArray(xFius)) return undefined;
    const stepLabel = (xFius as Record<string, unknown>).stepLabel;

...
import { readClipboardImage, readClipboardText, isSupportedFilePath, getFileMimeType } from '../utils/clipboardUtils.js';
            ...
            if (key.ctrl && key.name === 'v' && onImagePaste) {
                ...
                void (async () => {
                    try {
                        ...
                        // No image in clipboard - check if it's a file path
                        const clipboardText = await readClipboardText();
                        if (clipboardText && isSupportedFilePath(clipboardText)) {
                            const { readFileAsBase64 } = await import('../utils/clipboardUtils.js');
                            const base64 = readFileAsBase64(clipboardText);
                            const mimeType = getFileMimeType(clipboardText);

                            const currentCount = imageCountRef.current;
                            const imageNumber = currentCount + 1;
                            imageCountRef.current = imageNumber;

                            const placeholder = `[File ${imageNumber}]`;
                            const pendingImage: PendingImage = {
                        ...
                        // Nothing found - paste clipboard text normally
                        if (clipboardText) {

...
import { parseUnifiedDiff } from './renderers/diff-shared.js';
...
export const ApprovalPrompt = forwardRef<ApprovalPromptHandle, ApprovalPromptProps>(
    ({ approval, onApprove, onDeny, onCancel }, ref) => {
        ...
        // Calculate total diff lines for scroll bounds
        const displayPreviewRaw = approval.metadata.displayPreview as ToolDisplayData | undefined;
        const totalDiffLines = useMemo(() => {
            if (!displayPreviewRaw || displayPreviewRaw.type !== 'diff') return 0;
            try {
                const hunks = parseUnifiedDiff(displayPreviewRaw.unified);
                let count = 0;
        ...
        useImperativeHandle(
            ...
            () => ({
                handleInput: (input: string, key: Key) => {
                    ...
                    if (key.upArrow) {
                        if (key.shift) {
                            // Shift+Up: always navigate options
                            setSelectedIndex((current) =>
                                current === 0 ? totalOptions - 1 : current - 1
                            );
                        } else if (diffScrollTop > 0) {
                            // No Shift: scroll diff up if possible
                            setDiffScrollTop((prev) => Math.max(0, prev - 1));
                        } else {
                            // No Shift + diff at top: navigate options
                            setSelectedIndex((current) =>
                                current === 0 ? totalOptions - 1 : current - 1
                            );
                        }
                        ...
                        if (key.shift) {
                            // Shift+Down: always navigate options
                            setSelectedIndex((current) =>
                                current === totalOptions - 1 ? 0 : current + 1
                            );
                        } else if (diffScrollTop + DIFF_MAX_HEIGHT < totalDiffLines) {
                            // No Shift: scroll diff down if possible
                            setDiffScrollTop((prev) => prev + 1);
                        } else {
                            // No Shift + diff at bottom: navigate options
                            setSelectedIndex((current) =>
                                current === totalOptions - 1 ? 0 : current + 1
                            );
                        }
        ...
        const renderPreview = () => {
            ...
            switch (displayPreview.type) {
                case 'diff':
                    return <DiffPreview data={displayPreview} scrollTop={diffScrollTop} canScrollUp={diffScrollTop > 0} canScrollDown={diffScrollTop + DIFF_MAX_HEIGHT < totalDiffLines} />;
                case 'shell':
        ...
        return (
            <Box paddingX={0} paddingY={0} flexDirection="column">
                ...
                <Box marginTop={0}>
                    <Text color="gray">{'  '}↑↓ scroll diff • Shift+↑↓ select • Enter confirm • Esc cancel</Text>
                </Box>
            </Box>
        );
    }

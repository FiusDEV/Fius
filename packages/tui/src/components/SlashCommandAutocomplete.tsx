...
>(function SlashCommandAutocomplete(
    ...
    useEffect(() => {
        ...
        const fetchCommands = async () => {
            try {
                // Fetch prompts (filter out non-user-invocable)
                const promptList: PromptItem[] = supportsPrompts(agent)
                    ? Object.values(await agent.listPrompts())
                          .filter((p) => p.userInvocable !== false)
                          .map((p) => ({
                                ...p,

...
export function useCLIState({
    ...
    // Load last used model from model picker state on mount
    // Also switch the agent's actual LLM config (not just display name)
    useEffect(() => {
        let cancelled = false;
        ...
        (async () => {
            try {
                ...
                if (pickerState.recents.length > 0) {
                    ...
                    // Switch LLM WITH session ID to update the active session's VercelLLMService
                ...
                // Ignore errors — agent will use default config
            }
        })();

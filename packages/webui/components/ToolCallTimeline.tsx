...
export function ToolCallTimeline({
    ...
    const effectivePendingApproval = isPendingApproval && !localApproved;
    const isFailed = success === false;
    const isRejected = approvalStatus === 'rejected';
    const isProcessing = !hasResult && !effectivePendingApproval && !isFailed;
    ...
    const [expanded, setExpanded] = useState(
        effectivePendingApproval || (hasRichUI && !isFailed && !isRejected)
    );
    ...
    const StatusIcon = effectivePendingApproval ? (
    ...
    const toggleExpanded = () => {
        if (hasResult || effectivePendingApproval || hasExpandableContent) {
            setExpanded(!expanded);
        }
    };

    const canExpand = hasResult || effectivePendingApproval || hasExpandableContent;

    return (
        <div
            className={cn(
                'my-0.5 rounded-md transition-colors max-w-full',
                effectivePendingApproval ? 'block w-full' : 'inline-block',
                effectivePendingApproval &&
        ...
        >
            ...
            <button
                ...
                className={cn(
                    ...
                    effectivePendingApproval ? 'flex w-full' : 'inline-flex',
            ...
            >
                ...
                <span
                    className={cn(
                        ...
                        effectivePendingApproval && 'text-amber-700 dark:text-amber-300 font-medium',
                ...
                >
                        {effectivePendingApproval ? 'Approval required: ' : ''}
                    {isFailed ? 'Failed: ' : ''}
                    {isRejected ? 'Rejected: ' : ''}
                    {hasSubAgentProgress ? (
                    ...
                    {effectivePendingApproval && (
                        <>
                            ...
                            <div className="flex gap-1 flex-wrap pt-1">
                                <Button
                                    onClick={() => { setLocalApproved(true); onApprove?.(undefined, false); }}
                                    size="sm"
                                    className="h-6 text-[11px] px-2.5 bg-emerald-600 hover:bg-emerald-700 text-white border-0"
                                >
                                    {localApproved ? '✓ Approved' : 'Approve'}
                                </Button>
                                <Button
                                    onClick={() => { setLocalApproved(true); onApprove?.(undefined, true); }}
                                    size="sm"
                                    variant="ghost"
                                ...
                                <Button
                                    ...
                                    variant="ghost"
                                    size="sm"
                                    className="h-6 text-[11px] px-2.5 text-red-500 hover:bg-red-500/10"
                                >
                                    Reject
                                </Button>
                            </div>

...
export function InlineApprovalCard({ approval, onApprove, onDeny }: InlineApprovalCardProps) {
    ...
    return (
        <div className={`space-y-4 ${localApproved ? 'opacity-70' : ''}`}>
            {/* Header */}
            <div className="flex items-center gap-2 text-muted-foreground">
                ...
                {!localApproved && (
                    <span className="ml-auto text-xs text-muted-foreground">[mcp]</span>
                )}
                {localApproved && (
                    <span className="ml-auto text-xs text-emerald-500">[mcp] approved</span>
                )}
            </div>
                ...
                <div className="space-y-4 min-w-0">
                    <div className="bg-muted/50 p-3 rounded-md border border-border min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground break-words">
                            From: {approval.metadata.serverName || 'Fius Agent'}
                        </p>
                    </div>
            ...
            {!localApproved && (
                <div className="flex gap-1.5 justify-end pt-3 border-t border-border">
                    <Button variant="ghost" onClick={onDeny} size="sm" className="text-red-500 hover:bg-red-500/10">
                        {approval.type === ApprovalType.ELICITATION ? 'Decline' : 'Deny'}
                    </Button>
                    <Button variant="ghost" onClick={handleApprove} size="sm" className="text-emerald-500 hover:bg-emerald-500/10">

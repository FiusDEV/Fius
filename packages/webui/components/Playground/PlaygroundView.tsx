...
export default function PlaygroundView() {
    ...
    const [isGitHubMcpOpen, setIsGitHubMcpOpen] = useState(false);
    ...
    const shareToolConfig = () => {
        if (!selectedTool || !selectedServer) return;
        const shareText = `Check out this Fius tool configuration:\n\nServer: ${selectedServer.name}\nTool: ${selectedTool.name}\nInputs: ${JSON.stringify(toolInputs, null, 2)}`;
        if (navigator.share) {
            navigator.share({
                title: `Fius Tool: ${selectedTool.name}`,
                text: shareText,
            });
        } else {
    ...
    return (
        <div className="flex h-screen bg-background text-foreground antialiased">
                ...
                {showServersSidebar && (
                    <>
                        ...
                        <ServersList
                            ...
                            onOpenGitHubMcp={() => setIsGitHubMcpOpen(true)}
                        />
                    </>
                )}
            </aside>
            ...
            />
            <GitHubMcpBrowser
                isOpen={isGitHubMcpOpen}
                onClose={() => setIsGitHubMcpOpen(false)}
                onInstall={(server, hint) => {
                    setIsGitHubMcpOpen(false);
                    const cmd = hint?.command || '';
                    const args = hint?.args || [];
                    const name = server.name || 'mcp-server';

                    const msg = cmd
                        ? `Install the MCP server "${name}". Use the add_mcp_server tool with type "stdio", command "${cmd}", and args ${JSON.stringify(args)}.`
                        : `Install the MCP server "${name}" from ${server.repoUrl} using the addMcpServer tool.`;
                    navigate({
                        to: '/',
                        search: { installMcp: msg },
                    });
                }}
            />
        </div>

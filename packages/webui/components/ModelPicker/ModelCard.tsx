...
interface ModelCardProps {
    ...
    isInstalled?: boolean;
    /** Custom provider display name (e.g., "api.pateway.ai") */
...
export function ModelCard({
    ...
    const { providerName: rawProviderName, modelName, suffix } = parseModelName(displayName, provider);
    const providerName = displayProvider || rawProviderName;
    ...
    const sizeClasses = {
        sm: 'px-2 py-3 h-[160px] w-full',
        md: 'px-3 py-4 h-[190px] w-full',
        lg: 'px-4 py-5 h-[230px] w-full',
    };

    const logoSizes = {
        sm: { width: 28, height: 28, container: 'w-8 h-8' },
        md: { width: 36, height: 36, container: 'w-10 h-10' },
        lg: { width: 48, height: 48, container: 'w-14 h-14' },
    };
            ...
            <Tooltip>
                <TooltipTrigger asChild>
                    ...
                    >
                        ...
                        {/* Action Buttons - Top Right for custom/installed models */}
                        {(isCustom || isInstalled) && (onEdit || onDelete) && (
                            <div className="absolute top-1.5 right-1.5 flex gap-0.5 z-10 opacity-0 group-hover:opacity-100 transition-all duration-200">
                                {onEdit && (
                                    <button
                                        onClick={(e) => {
                                            e.stopPropagation();
                                    ...
                                    >
                                        <Pencil className="h-3 w-3 text-muted-foreground/60 hover:text-primary" />
                                    </button>
                                )}
                                {onDelete && (
                                    <button
                        ...
                        <button
                            ...
                            className={cn(
                                'absolute top-1.5 left-1.5 p-1 rounded-md transition-all duration-200 z-10',
                                'bg-background/80 backdrop-blur-sm',
                        ...
                        <div className="mt-auto pt-2 h-8 flex items-center justify-center overflow-hidden">
                            <CapabilityIcons

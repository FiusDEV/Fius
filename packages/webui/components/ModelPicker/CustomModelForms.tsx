...
import { lookupModelCapabilities, getSupportedFileTypesFromModelsDev, type ModelCapabilities } from './models-dev-lookup';

const BEDROCK_DOCS_URL = 'https://docs.fius.ai/docs/guides/supported-llm-providers#amazon-bedrock';
...
function OpenAICompatibleFields({
    ...
    return (
        <>
            ...
            <div className="space-y-1.5">
                <label className="text-xs font-medium text-muted-foreground">
                    Max Tokens <span className="text-muted-foreground/60">(optional)</span>
                </label>
                <Input
                ...
                />
                <p className="text-[10px] text-muted-foreground">
                    Maximum tokens for model output. Leave empty to use provider default.
                </p>
            </div>
        </>
    );
...
/**
 * Fius fields - model ID only, no API key (uses OAuth login)
 */
function FiusFields({ formData, onChange, setLocalError }: ProviderFieldsProps) {
    return (
        <>
            {/* Model ID */}
            <div className="space-y-1.5">
            ...
            {/* No API Key field - Fius uses OAuth login */}
        </>
    );
}
...
export function CustomModelForm({
    ...
    const providerOptions = [
        ...
        ...customProviders.map((cp) => ({
            value: cp as CustomModelProvider,
            label: cp,
    ...
    useEffect(() => {
        if (isEditing && formData.supportedFileTypes && formData.supportedFileTypes.length > 0) {
            setDetectedCapabilities({
                fileTypes: formData.supportedFileTypes,
                tools: false,
                reasoning: false,
                structured: false,
                temperature: false,
    ...
    const [detectedCapabilities, setDetectedCapabilities] = useState<ModelCapabilities | null>(null);
    const [isDetecting, setIsDetecting] = useState(false);
    const detectTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
    const lastLookupKeyRef = useRef<string>('');

    const displayProviderForLookup = customProviders.includes(formData.provider)
        ? formData.provider
        : (formData.provider === 'fius' ? '' : formData.provider);
    ...
    useEffect(() => {
        ...
        const provider = displayProviderForLookup?.trim();

        if (!modelName || !provider || isEditing) {
            setDetectedCapabilities(null);
            return;
        }

        const lookupKey = `${provider}|${modelName}`;
        lastLookupKeyRef.current = lookupKey;

        detectTimerRef.current = setTimeout(async () => {
            setIsDetecting(true);
            try {
                const caps = await lookupModelCapabilities(provider, modelName);
                if (lastLookupKeyRef.current === lookupKey) {
                    if (caps) {
                        setDetectedCapabilities(caps);
                        if (caps.fileTypes.length > 0) {
                            onChange({ supportedFileTypes: caps.fileTypes });
                        }
                    } else {
                        setDetectedCapabilities(null);
                    }
    ...
    }, [formData.name, displayProviderForLookup, isEditing]);
    ...
    return (
        <div className="flex flex-col flex-1 min-h-0 max-h-full">
            ...
            <div className="flex-1 min-h-0 overflow-y-auto p-4 space-y-4">
                ...
                {(isDetecting || (detectedCapabilities && formData.name.trim())) && (
                    <div className="space-y-1.5">
                        <label className="text-xs font-medium text-muted-foreground">
                        ...
                        <div className="flex items-center gap-1.5 flex-wrap min-h-[32px]">
                            {isDetecting ? (
                                <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                                    ...
                                    <span>Detecting from models.dev...</span>
                                </div>
                            ) : detectedCapabilities ? (
                                <>
                                    <Scan className="h-3 w-3 text-emerald-500" />
                                    {detectedCapabilities.tools && (
                                    ...
                                    {detectedCapabilities.structured && (
                                        <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20">
                                            📋 Structured
                                        </span>
                                    )}
                                    {detectedCapabilities.temperature && (
                                        <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20">
                                            🌡 Temperature
                                        </span>
                                    )}
                                    {detectedCapabilities.attachment && (
                                        <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20">
                                            📎 Attachment
                                        </span>
                                    )}
                                    {detectedCapabilities.fileTypes.includes('image') && (
                                        <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20">
                                            🖼 Vision
                                        </span>
                                    )}
                                    {detectedCapabilities.fileTypes.includes('pdf') && (
                                        <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20">
                                            📄 PDF
                                        </span>
                                    )}
                                    {detectedCapabilities.fileTypes.includes('audio') && (
                                        <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20">
                                            🎙 Audio
                                        </span>
                                    )}
                                    {detectedCapabilities.fileTypes.includes('video') && (
                                        <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20">
                                            🎬 Video
                                        </span>
                                    )}

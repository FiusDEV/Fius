...
): FiusRuntimeError<Record<string, unknown> | undefined> {
...
function normalizeCodexStartupError(error: unknown, command: string): Error {
    ...
    const message =
        command === 'codex'
            ? 'Codex CLI not found on PATH. Install Codex to use ChatGPT Login in Fius.'
            : `Codex CLI command '${command}' not found on PATH. Install Codex to use ChatGPT Login in Fius.`;
...
export function createCodexLanguageModel(options: {
    ...
    async function executeTurn(callOptions: LanguageModelV2CallOptions): Promise<{
        ...
        try {
            ...
            if (
                mappedError instanceof FiusRuntimeError &&
